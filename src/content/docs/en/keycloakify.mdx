...
1. Themes are defined in a file format called freemarker template (`.ftl`). An example of one the built-in theme pages looks like this (`info.ftl`):
```html
...
- Access Runtime Keycloak Environment Variables by adding a `keycloakify.extraThemeProperties`
